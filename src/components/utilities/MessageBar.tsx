...
const MessageBar = () => {
  ...
  const handleSendMsg = () => {
    ...
    let conn_type = currentFriend?.conn_type as "group" | "onetoone";

    if (message.trim() !== '' && currentUserIndex !== null && currentFriend) {
      ...
      // Simpan/tampilkan pesan di state Redux lokal
      ...
      <form autoFocus={true} className="w-full" onSubmit={onSubmit}>
        <div className="bg-[#202c33] text-white px-2 flex items-center gap-2 sm:gap-6">
          <div className="flex">
            ...
            <div onClick={() => { dispatch(setShowAttachFiles(!showAttachFiles)); }} className="icons">
          ...
          <div className="w-full rounded-lg py-2 flex items-center">
            <input
              ...
              onChange={handleSendMsgFunction}
              value={message}
            />
          </div>
